...
export function composePrompt(context: PromptContext): string {
  ...
  if (context.stage === "implement" || context.stage === "repair") {
    ...
    common.push(
      ...
      'When browser_dom_validation is approved, you may choose one task-specific browserProbe from the advertised families, profiles, and action DSL. Include browser_dom_validation in requiredCapabilities. A dom_security probe must include {"kind":"fill_dom_xss_canary","label":"accessible field label"}; the harness supplies and observes the inert payload. A browser-only attack may omit focusedCommand and paths; it lands only if the harness reproduces an author pass and target failure and the judge accepts its oracle. The harness also runs mandatory runtime, semantics, and 320-pixel smoke probes.',
