...
const FightConfigBaseSchema = z
  .object({
    ...
    attackVerifierModel: z.string().trim().min(1).optional(),
    qualityVerifier: AgentIdSchema.optional(),
    qualityVerifierModel: z.string().trim().min(1).optional(),
    harnessMaintainer: AgentIdSchema,
    harnessMaintainerModel: z.string().trim().min(1).optional(),
    rounds: z.literal(3),
...
const LegacyContestantResultSchema = ContestantResultSchema.omit({
  id: true,
  provider: true,
  role: true,
